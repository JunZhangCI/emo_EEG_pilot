...
sampl_freq = eeg_to_be_corrected.srate;
labels = {eeg_to_be_corrected.chanlocs.labels};
types = {eeg_to_be_corrected.chanlocs.type};
eeg_idx = find(strcmpi(types, 'EEG'));
eeg_labels = labels(eeg_idx);
channels_to_plot = {'F3', 'Fz', 'F4', 'FC1', 'FC2', 'C3', 'Cz', 'C4'};
...
for kk = 1:size(eeg_to_be_corrected_data,1)
    ...
    % Only plot if this channel is in the list
    if ismember(eeg_labels{kk}, channels_to_plot) 
        figure 
        subplot(2,1,1)
