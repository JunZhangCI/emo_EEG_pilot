function [EEG_cleaned, rejectedStr, IC_shown] = run_sobi_manual_reject_v2( ...
...
while ~done
    ...
    %% Optional PRP curve plot before/after ICA
    choice_prp = questdlg( ...
        'Do you want to inspect PRP curves before and after ICA?', ...
        'Inspect PRP', ...
        'Yes','No','No');
    
    if strcmp(choice_prp, 'Yes')
        if isfield(EEG_uncleaned, 'phoneme_onset_index') && isfield(EEG_uncleaned, 'phonemes')
            try
                figure('Name', 'PRP Before ICA', 'Color', 'w');
                [~, ~, ~, ~] = plot_manner_prp_curves_from_EEG( ...
                    EEG_uncleaned, ...
                    'Channels', existingCh, ...
                ...
                title(sprintf('Before ICA | focus channels: %s', strjoin(existingCh, ', ')));
    
                figure('Name', 'PRP After ICA', 'Color', 'w');
                [~, ~, ~, ~] = plot_manner_prp_curves_from_EEG( ...
                    EEG_cleaned, ...
                    'Channels', existingCh, ...
                    'BaselineDuration', 0.1, ...
                ...
                title(sprintf('After ICA | rejected ICs: %s', rejectedStr));
    
            catch ME
                warning('PRP plotting failed: %s', ME.message);
            end
        else
            warning('EEG does not contain phoneme_onset_index / phonemes fields. PRP plotting skipped.');
        end
    end
